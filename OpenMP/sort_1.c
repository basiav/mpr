...
#define BUCKETS_PER_THREAD 2500
...
int main(int argc, char** argv) {
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp master
        {
            ...
            threads_buckets_counts = (size_t*)malloc(sizeof(size_t) * num_threads);
        ...
        for(size_t i = 0; i < ARRAY_SIZE; i++) {
            // The bucket range interval is left-closed, right-open [my_min_range, my_max_range) to ensure nonoverlaps
        ...
        size_t thread_total_elements = 0;
        for(size_t i = 0; i < buckets_per_thread; i++) {
            qsort(thread_buckets[tid][i].elements, thread_buckets[tid][i].count, sizeof(int), compare_function);
            thread_total_elements += thread_buckets[tid][i].count;
        }
        MEASURE_TIME(t_sort_e);
        threads_buckets_counts[tid] = thread_total_elements;
        ...
        for (int i = 0; i < tid; i++) {
            write_offset += threads_buckets_counts[i];
        }
